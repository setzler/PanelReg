...
PR.est.AR1.GMM <- function(panel_data, outcome_name, endogenous_names, covariate_names = NULL, fixedeffect_names = NULL, AR1_IV_outcome = TRUE, AR1_IV_lags = 1, AR1_persistence = NULL) {
  if (is.null(AR1_persistence)) {
    ...
    return(PR.est.AR1.GMM.unknown_persistence(panel_data = panel_data, outcome_name = outcome_name, endogenous_names = endogenous_names, covariate_names = covariate_names, fixedeffect_names = fixedeffect_names, AR1_IV_lags = AR1_IV_lags, AR1_IV_outcome = AR1_IV_outcome, beta_guess = initial_guess$beta_params, AR1_persistence_guess = initial_guess$AR1_persistence, delta_guess = initial_guess$delta_params))
  } else {
...
PR.est.AR1.GMM.unknown_persistence.init_search <- function(panel_data, outcome_name, endogenous_names, covariate_names = NULL, fixedeffect_names = NULL, AR1_IV_lags = 1, AR1_IV_outcome = TRUE, AR1_persistence = NULL, num_restarts = 10) {
  ...
  # get intercept
  resid_it = panel_data[, get(outcome_name)] - as.matrix(panel_data[, .SD, .SDcols = endogenous_names]) %*% beta_guess
  if (n_exogenous > 0) {
    resid_it = resid_it - as.matrix(panel_data[, .SD, .SDcols = exogenous_names]) %*% delta_guess
  }
  intercept_guess = mean(resid_it)
  delta_guess = c(intercept_guess, delta_guess)
