...
```{r echo=T, eval=T, message=FALSE}
simulated_data = PanelRegSim(panel_model = "MA1", seed = 123, sample_size = 20000, 
...
```{r echo=T, eval=T, message=FALSE}
reg = PanelReg(panel_data = copy(simulated_data), panel_model = "MA1", varnames = varnames)
head(reg)
```

To control for covariates, we add their names to the varnames list in the `$covariate_names` argument:
...
Consider again the "iid" model. We can assign units to groups in the simulated model by setting the `true_FE_var` parameter:

```{r echo=T, eval=T, message=FALSE}
simulated_data = PanelRegSim(panel_model = "MA1", seed = 123, sample_size = 20000, 
                             noise_sd = 0, min_year = 2003, max_year = 2006, 
                             true_beta = c(0.5, -0.2), true_FE_var = 1)
head(simulated_data)
```
...
To control for group fixed-effects, we add their names to the varnames list in the `$fixedeffect_names` argument:

```{r echo=T, eval=T, message=FALSE}
varnames$fixedeffect_names = "group_id"
