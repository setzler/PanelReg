...
This package can simulate and estimate panel regression models of the following form:
...
2. Serially Independent ("iid"): $\epsilon_{it} = \eta_{it}$, where $\eta_{it}$ is exogenous.
3. Moving Average of Order 1 ("MA1"): $\epsilon_{it} = \eta_{it} + \theta \eta_{it-1}$, where $\eta_{it}$ is exogenous.
4. First Order Autoregressive  ("AR1"): $\epsilon_{it} = \rho \epsilon_{it-1} + \eta_{it}$, where $\eta_{it}$ is exogenous.
...
In order to use PanelReg, we must first define the variable names. The required variable names are:
...
- `covariate_names`: A vector of the names of the exogenous regressors. This is the only optional variable name.
...
The error term is given by $\epsilon_{it} = \alpha_i$. The data can be simulated as follows:


```{r echo=T, eval=T, message=FALSE}
simulated_data = PanelRegSim(panel_model = "FE", seed = 123, sample_size = 10000, 
...
```{r echo=T, eval=T, message=FALSE}
simulated_data = PanelRegSim(panel_model = "iid", seed = 123, sample_size = 10000, 
                             min_year = 2003, max_year = 2006, true_beta = c(0.5, -0.2))
...
```{r echo=T, eval=T, message=FALSE}
simulated_data = PanelRegSim(panel_model = "MA1", seed = 123, sample_size = 10000, 
                             min_year = 2003, max_year = 2006, true_beta = c(0.5, -0.2))
...
Estimating the model as if it were the "MA1" model, the biased estimates are as follows:
...
The package provides two strategies for estimating the AR(1) model: Panel IV and GMM.

**Panel IV:** The error structure implies that, $$ y_{it} = \rho y_{it-1} + x_{it}'\beta +  x_{it-1}'(-\beta\rho) +  x_{it}'\gamma + x_{it-1}'(-\gamma\rho) + \eta_{it}$$ Since only $x_{it}$ depends on the unobservable $\eta_{it}$, and lagged values of $x$ are independent of contemporaneous $\eta$, it follows that $\beta$ can be estimated by a regression of $y_{it}$ on $x_{it}$, with instrument $x_{it-2}$, controlling for $(y_{it-1}, x_{it-1}, w_{it-1})$. Another available instrument is $y_{it-2}$, so there is an option to use the lagged outcome as an IV as well. 
...
reg = PanelReg(panel_data = simulated_data, panel_model = "AR1", varnames = varnames, 
AR1_options = AR1_options)
...
Note that an estimate of the AR(1) persistence parameter, $\rho$, is returned by both method "AR1".



## 7. Incorporating Exogenous Covariates

Consider again the "AR1" model. We can add covariates to the simulated model by setting the `true_gamma` parameter:

```{r echo=T, eval=T, message=FALSE}
simulated_data = PanelRegSim(panel_model = "AR1", seed = 123, sample_size = 20000, 
                             noise_sd = 0, min_year = 2003, max_year = 2009, 
                             true_beta = c(0.5, -0.2), true_gamma = c(1, -0.5))
...
reg = PanelReg(panel_data = copy(simulated_data), panel_model = "AR1", varnames = varnames, AR1_options = AR1_options)

head(reg)

```

To control for covariates, we add their names to the varnames list:
